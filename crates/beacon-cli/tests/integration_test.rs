...
use beacon_core::{Planner, PlannerBuilder, PlanFilter, PlanStatus};
...
/// Test that plan creation has consistent output between CLI and direct Display impl
#[tokio::test] 
async fn test_plan_display_consistency() {
    ...
    // Create plan via CLI
    let cli_output = run_cli_command(db_str, &[
        "plan", "create", "Integration Test Plan",
...
async fn test_step_display_consistency() {
    ...
    let _plan_output = run_cli_command(db_str, &[
        "plan", "create", "Step Test Plan"
    ]);
    
    // Add step via CLI
    let cli_output = run_cli_command(db_str, &[
        "step", "add", "1", "Test Step", 
        "--description", "Step added via CLI"
    ]);
    ...
    let plan = planner.create_plan(&plan_params).await.expect("Failed to create plan");
...
async fn test_list_plans_consistency() {
    ...
    let _output1 = run_cli_command(db_str, &[
        "plan", "create", "List Test Plan 1"
    ]);
    let _output2 = run_cli_command(db_str, &[
        "plan", "create", "List Test Plan 2",
    ...
    let plan1 = planner.create_plan(&plan_params1).await.expect("Failed to create plan");
    let plan2 = planner.create_plan(&plan_params2).await.expect("Failed to create plan");
...
async fn test_show_plan_consistency() {
    ...
    let _plan_output = run_cli_command(db_str, &[
        "plan", "create", "Show Test Plan",
        "--description", "Plan for show testing"
    ]);
    
    let _step_output = run_cli_command(db_str, &[
        "step", "add", "1", "Test Step",
        "--description", "Step for testing"
    ]);
    ...
    let _step = planner.add_step(&step_params).await.expect("Failed to add step");
    
    // Get plan with steps
    let params = beacon_core::params::Id { id: plan.id };
    let mut full_plan = planner.get_plan(&params).await.expect("Failed to get plan").expect("Plan not found");
    full_plan.steps = planner.get_steps(&params).await.expect("Failed to get steps");
...
async fn test_show_step_consistency() {
    ...
    let _plan_output = run_cli_command(db_str, &[
        "plan", "create", "Step Show Test Plan"
    ]);
    
    let _step_output = run_cli_command(db_str, &[
        "step", "add", "1", "Show Step Test",
        "--description", "Detailed step description",
        "--acceptance-criteria", "Should show all fields correctly"
    ]);
...
/// Test CLI vs MCP-style list output (simulating what the MCP server would return)
#[tokio::test]
async fn test_cli_vs_mcp_list_output() {
    ...
    // Create some plans via CLI
    let _output1 = run_cli_command(db_str, &[
        "plan", "create", "MCP Test Plan 1", 
        "--description", "First test plan"
    ]);
    let _output2 = run_cli_command(db_str, &[
        "plan", "create", "MCP Test Plan 2"
    ]);
    ...
    let plans = planner.list_plans(filter).await.expect("Failed to list plans");
    
    // Convert to summaries as the MCP server would
    let mut plan_summaries = Vec::new();
    for plan in plans {
        let steps = planner.get_steps(&beacon_core::params::Id { id: plan.id })
            .await
            .expect("Failed to get steps");
        
        let completed_steps = steps.iter().filter(|s| s.status == beacon_core::StepStatus::Done).count() as u32;
    ...
    // The core formatting should be identical (ignoring potential whitespace differences)
    let cli_lines: Vec<&str> = cli_list.lines().map(|l| l.trim()).collect();
    let mcp_lines: Vec<&str> = mcp_list_str.lines().map(|l| l.trim()).collect();
    
    assert_eq!(cli_lines.len(), mcp_lines.len(), "Different number of output lines");
...
async fn test_cli_vs_mcp_show_plan_output() {
    ...
    let _plan_output = run_cli_command(db_str, &[
        "plan", "create", "MCP Show Plan",
        "--description", "Plan for MCP comparison testing"
    ]);
    
    let _step1_output = run_cli_command(db_str, &[
        "step", "add", "1", "First Step",
        "--description", "First step description"
    ]);
    
    let _step2_output = run_cli_command(db_str, &[
        "step", "add", "1", "Second Step",
        "--acceptance-criteria", "Should complete successfully"
    ]);
    ...
    let mut plan = planner.get_plan(&params)
        .await
        .expect("Failed to get plan")
        .expect("Plan not found");
        
    plan.steps = planner.get_steps(&params)
        .await
        .expect("Failed to get steps");
...
async fn test_cli_vs_mcp_show_step_output() {
    ...
    // Create plan and step via CLI
    let _plan_output = run_cli_command(db_str, &[
        "plan", "create", "Step Comparison Plan"
    ]);
    
    let _step_output = run_cli_command(db_str, &[
        "step", "add", "1", "Comparison Step",
        "--description", "Step for CLI vs MCP comparison",
        "--acceptance-criteria", "Both outputs should match exactly",
        "--references", "doc1.md,https://example.com"
    ]);
    ...
    let params = beacon_core::params::Id { id: 1 };
    let step = planner.get_step(&params)
        .await
